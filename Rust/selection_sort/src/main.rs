...
mod test{
    ...
    fn reverse_test(){
        ...
        assert_eq!(ans_vector, selection_sort(test_vector));
    }
    #[test]
    fn no_sort_test(){
...
fn main() {
    ...
    match io::stdin().read_line(&mut input) {
        Ok(_n) => {
            ...
            output = selection_sort(nums);
            println!("{:?}", output);
        }
        Err(error) => println!("error: {}. Did not recieve a valid line!", error),
    }
}
fn selection_sort(input_vector: Vec<i32>) -> Vec<i32>{
    let mut processing_vector: Vec<i32> = input_vector;
    let mut lowest;
    let mut hold;
    for i in 0..processing_vector.len(){
        lowest = i.clone();
        for j in i..processing_vector.len(){
            if processing_vector[j]<processing_vector[lowest]{
                lowest = j;
            }       
        }
        hold = processing_vector[i];
        processing_vector[i] = processing_vector[lowest];
        processing_vector[lowest] = hold;
    }
